...
def validate_book(path, data, errors, warnings, seen_ids):
    ...
    # المراجع المترجَمة (حقل اختياري)
    if data.get("مترجَم") in ("نعم", True):
        if not data.get("المترجِم"):
            err("الكتاب موسوم «مترجَم» لكن حقل «المترجِم» مفقود")
        if not data.get("جهة_الترجمة"):
            warn("الكتاب مترجَم بلا «جهة_الترجمة» (المنظمة العربية للترجمة، كلمة...)")
        if nau != "معاصرة":
            warn("المراجع المترجَمة الأنسب أن يكون نوع_مرجعيتها «معاصرة»")
